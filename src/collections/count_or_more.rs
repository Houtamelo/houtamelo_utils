...
#[cfg_attr(
	...
	serde(bound(
		serialize = "T: serde::Serialize, [T; COUNT]: serde::Serialize"
	)),
	serde(bound(deserialize = "T: serde::de::DeserializeOwned, [T; \
	                           COUNT]: serde::de::DeserializeOwned"))
)]
...
impl<const COUNT: usize, T> CountOrMore<COUNT, T> {
	...
	pub fn iter(&self) -> impl Iterator<Item = &T> {
		self.fixed.iter().chain(self.dynamic.iter())
	}

	pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut T> {
		self.fixed.iter_mut().chain(self.dynamic.iter_mut())
	}

	#[must_use]
	pub fn get(&self, index:usize) -> Option<&T> {
		if index < COUNT {
			Some(&self.fixed[index])
		} else {
			self.dynamic.get(index - COUNT)
		}
	}
	...
	pub fn get_mut(&mut self, index:usize) -> Option<&mut T> {
		if index < COUNT {
			Some(&mut self.fixed[index])
		} else {
			self.dynamic.get_mut(index - COUNT)
		}
	}

	pub fn clear_extras(&mut self) { self.dynamic.clear(); }

	pub fn push(&mut self, value:T) { self.dynamic.push(value); }

	pub fn append(&mut self, other:&mut Vec<T>) { self.dynamic.append(other); }

	pub fn extend(&mut self, other:impl IntoIterator<Item = T>) {
		self.dynamic.extend(other);
	}

	pub fn pop(&mut self) -> Option<T> { self.dynamic.pop() }

	/// ### Returns
	/// The value trying to be inserted, if it was not inserted. (Due to being
	/// out of bounds)
	#[must_use]
	pub fn insert(&mut self, index:usize, value:T) -> Option<T> {
		...
		if index < COUNT {
			...
			let fixed_to_move_to_dyn = std::mem::replace(
				&mut self.fixed[index],
				value,
			);
			self.dynamic.insert(
				0,
				fixed_to_move_to_dyn,
			);
		} else {
	...
	pub fn remove(&mut self, index:usize) -> Option<T> {
		if index >= self.len() || self.dynamic.is_empty() {
			return None;
		}
	...
	pub fn swap_remove(&mut self, index:usize) -> Option<T> {
		...
		if index < COUNT {
			let result = std::mem::replace(
				&mut self.fixed[index],
				self.dynamic.remove(self.dynamic.len() - 1),
			);
			Some(result)
		} else {
	...
	pub fn shrink_to_fit(&mut self) { self.dynamic.shrink_to_fit(); }

	#[must_use]
	pub fn len(&self) -> usize { COUNT + self.dynamic.len() }

	#[must_use]
	pub fn is_empty(&self) -> bool { COUNT == 0 && self.dynamic.is_empty() }

	#[must_use]
	pub fn capacity(&self) -> usize { COUNT + self.dynamic.capacity() }

	pub fn push_within_capacity(&mut self, value:T) -> Result<(), T> {
		self.dynamic.push_within_capacity(value)
	}

	#[must_use]
	pub fn contains(&self, value:&T) -> bool
	where T: Ord {
		self.fixed.contains(value) || self.dynamic.contains(value)
	}
	...
	pub fn try_reserve(
		...
		self.dynamic.try_reserve(additional)
	}
	...
	pub fn try_reserve_exact(
		...
		self.dynamic.try_reserve_exact(additional)
	}
	...
	where
		...
		K: Ord,
	{
		let mut sorted_idx_var:Vec<_> = self.iter().enumerate().collect();

		sorted_idx_var.sort_by_key(|(_, x)| f(x));

		let mut old_idxs:Vec<usize> = sorted_idx_var
			.into_iter()
	...
	#[must_use]
	pub fn into_sorted_by_key<K, F>(self, mut f:F) -> Self
	where
		F: FnMut(&T) -> K,
		K: Ord,
	{
		let mut combined:Vec<T> = self.into_iter().collect();

		combined.sort_by_key(|x| f(x));

		let fixed:[T; COUNT] = [(); COUNT].map(|_| combined.remove(0));
		let dynamic = combined;
		CountOrMore::new(fixed, dynamic)
	}
	...
	pub fn swap(&mut self, a_idx:usize, b_idx:usize) {
		...
		if a_idx < COUNT && b_idx < COUNT {
			...
			let a_var = std::mem::replace(
				&mut self.fixed[a_idx],
				self.dynamic.remove(b_idx_dyn),
			);
			...
			let b = std::mem::replace(
				&mut self.fixed[b_var],
				self.dynamic.remove(a_idx_dyn),
			);
	...
	where F: FnMut(&T) -> bool {
		...
		retained.into_iter().enumerate().for_each(
			|(origin_idx, retained_idx)| {
				self.swap(
					origin_idx,
					retained_idx,
				)
...
impl<const COUNT: usize, T> IntoIterator for CountOrMore<COUNT, T> {
	type IntoIter =
		Chain<std::array::IntoIter<T, COUNT>, std::vec::IntoIter<T>>;
	type Item = T;

	fn into_iter(self) -> Self::IntoIter {
		self.fixed.into_iter().chain(self.dynamic)
	}
}

impl<'a, const COUNT: usize, T> IntoIterator for &'a CountOrMore<COUNT, T> {
...
impl<'a, const COUNT: usize, T> IntoIterator for &'a mut CountOrMore<COUNT, T> {
	type IntoIter =
		Chain<core::slice::IterMut<'a, T>, core::slice::IterMut<'a, T>>;
	type Item = &'a mut T;
...
mod tests {
	...
	fn test_new() {
		...
		let count_or_more = CountOrMore::new(
			fixed,
			dynamic.clone(),
		);
	...
	fn test_iter_mut() {
		...
		let mut count_or_more = CountOrMore::new(
			fixed,
			dynamic.clone(),
		);
	...
	fn test_extend() {
		...
		assert_eq!(
			count_or_more.get(11),
			Some(&12)
		);
		assert_eq!(
			count_or_more.get(12),
			Some(&13)
		);
	}
	...
	fn test_remove() {
		...
		assert_eq!(
			count_or_more.remove(4),
			Some(10)
		);
		assert_eq!(
			count_or_more.remove(10),
			None
		);

		assert_eq!(
			count_or_more.remove(0),
			Some(5)
		);
		assert_eq!(
			count_or_more.fixed,
	...
	fn test_sort_by_key() {
		...
		assert_eq!(
			count_or_more.into_iter().collect::<Vec<_>>(),
			vec![1, 2, 3, 4, 5]
		);
		...
		assert_eq!(
			count_or_more.into_iter().collect::<Vec<_>>(),
			vec![0, 1, 2, 3, 5, 10, 20, 25, 36, 40, 80]
		);
	...
	fn test_try_retain() {
		...
		assert!(count_or_more.try_retain(|x| *x < 6));
