...
where &'a Iter: IntoIterator<Item = &'a Item>
{
    fn get_weighted_random(&'a self, rng: &mut impl Rng) -> Option<&'a T> {
        let weight_sum = self.into_iter().fold(0., |acc, val_weight| acc + val_weight.weight());

        let random = rng.random_range(0.0..=weight_sum);
