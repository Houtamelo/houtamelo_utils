...
impl<T> IndexedSet<T> where T : Hash + PartialEq + Eq {
	...
	pub fn drain<R>(&mut self, range: R) -> Drain<T> where R: RangeBounds<usize> {
		return self.inner.drain(range);
	}
	
	pub fn get_range(&self, range: &RangeInclusive<usize>) -> Option<&[T]> {
		return self.inner.get(range.clone());
	}
	...
	pub fn swap_remove(&mut self, index: usize) -> T {
		return self.inner.swap_remove(index);
	}
	...
	pub fn sort_by<F>(&mut self, compare: F) where F: FnMut(&T, &T) -> std::cmp::Ordering {
	...
	/// assert!(v == [5, 4, 3, 2, 1]);
	pub fn sort_unstable_by<F>(&mut self, compare: F) where F: FnMut(&T, &T) -> std::cmp::Ordering {
...
impl<T> std::ops::Index<usize> for IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Output = T;
...
impl<T> std::ops::Index<usize> for &mut IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Output = T;
...
impl<T> std::ops::Index<&T> for IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Output = T;
	
	fn index(&self, key: &T) -> &Self::Output {
		return self.inner.iter().find(|&value| value == key).unwrap();
	}
}
...
impl<T> std::ops::Index<&T> for &mut IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Output = T;
...
impl<T> IntoIterator for IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Item = T;
...
impl<'a, T> IntoIterator for &'a IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut IndexedSet<T> where T : Hash + PartialEq + Eq {
	type Item = &'a mut T;
...
impl<T> Extend<T> for IndexedSet<T> where T : Hash + PartialEq + Eq {
...
impl<'a, T> IterGenerator<'a, &'a T> for IndexedSet<T> where T : Hash + PartialEq + Eq {
	fn iterate(&'a self) -> impl ExactSizeIterator<Item = &'a T> {
		return self.inner.iter();
	}
}

impl<'a, T> IterGenerator<'a, &'a T> for &'a IndexedSet<T> where T : Hash + PartialEq + Eq {
	fn iterate(&'a self) -> impl ExactSizeIterator<Item = &'a T> {
		return self.inner.iter();
	}
}

impl<'a, T> IterGenerator<'a, &'a T> for &'a mut IndexedSet<T> where T : Hash + PartialEq + Eq {
	fn iterate(&'a self) -> impl ExactSizeIterator<Item = &'a T> {
		return self.inner.iter();
	}
}
